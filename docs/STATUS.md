...
Plan: `BUILD-PLAN.md` · Rules: `RULES.md` (rev 16) · Skill: `.claude/skills/poker/SKILL.md`
...
#27**) and,
unlike everything else in §9, they are not fidelity questions with safe defaults — they are what
the rule *means*. **`QUESTIONS-FOR-MYA-LAY.md` Q4 carries them**, and Mya Lay is the
person who raised the rule. See **Decisions needed from Nick**.
...
🔥 **The next packet is P24 — the computer's reasoning, said out loud** (planned 2026-08-20,
`BUILD-PLAN.md` §5). P0–P23 are all done and every goal in §0 is delivered — the rules core, a
...
blocked on four unrecorded specification questions (§9 #16–#19) and P24 is what makes that conversation
...
🔥 **One, and it is new: `RULES.md` §5.1 — the feeding ban.** Mya Lay ruled on it directly at a
playtest on 2026-08-20 and it is `EXPERT`, Settled: **you may not discard a rank the next player
has taken in the open**, until they discard that rank themselves (which frees it for the rest of
the round, permanently) or you are going out on it. **Nothing in the code enforces it** — this is
the first rule in the document that constrains *which* card you may discard, and `RoundEngine`
accepts any of the fourteen.
✅ **How it is enforced is settled — two rulings by Nick on 2026-08-20, and §9 #20 was raised and
closed between them.** *(a)* A banned discard is **not an infraction but an impossible move**:
never offered, cannot be chosen, so there is no penalty and nothing to retract. *(b)* Where the ban
would leave a player **no legal discard at all**, **the ban yields for that turn** — the discard is
mandatory (§7.1), the ban is not. **(b) is what makes (a) safe**, and together they are one line:
the legal discards are *the hand minus the banned ranks, or the whole hand if that is empty*,
**never empty by construction**. ⚠️ It is **not** §9 #6's unrecovered exception to the mandatory
discard — it is the reverse. ⚠️ **Rev 17 then closed #6 outright: there is no exception, you always discard**, which settles the speculation the same way and makes the floor the only way §5.1 and the mandatory discard can both hold.
⚠️ **Six details are still unrecorded and they are not fidelity questions** — they are the rule's
specification, and the packet that implements it cannot compile without answering them:
`RULES.md` §9 **#16–#19, #25 and #27** (who is bound, what arms it, money card or any card,
whether a release survives a reshuffle, whether the rule works **two-handed** where the seat that
feeds you is the seat you feed, and what taking a **joker** closes given that a joker has no
rank). Each has a recommendation; **`QUESTIONS-FOR-MYA-LAY.md` Q4 has all
six phrased flat**, and she is the person who raised the rule, so this is one conversation rather
than a research programme.

**Every earlier blocking rules question is closed** (`RULES.md` rev 10). P0 through P23 all
proceeded without further input.
