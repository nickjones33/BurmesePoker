...
can move. ✅ **`P51` (containerize) shipped 2026-08-28** — Dockerfile, `0.0.0.0:8080`,
`UseForwardedHeaders`, `/healthz`, and a real round played inside the container. **`P52` (a
published image) is next and is blocked on a Gitea PAT.** ⚠️ **P52 must build the Dockerfile as it
stands**: P51 measured that a `--no-restore` publish drops `blazor.web.js` and ships a table that
renders perfectly and never moves, so its acceptance includes curling
`_framework/blazor.web.js` for a 200, not merely `/healthz`. ⚠️ **Docker and Ansible
