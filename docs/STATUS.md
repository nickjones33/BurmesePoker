...
🔥 **`P45` shipped 2026-08-25 on Fable 5: `angler` — a draw priced in cards — and the answer is
the predicted null with P44's shape: the mechanism never armed.** `RULES.md` stays rev 31 (no
rule changed), the tree is green at **886**, and **`P46` `sprinter` is the next packet**
(`BUILD-PLAN.md` §5 — then P47's ledger is already landed, P48's verification run, and the
documents; `P40` stands beside them, blocked on Nick's vetted Burmese text). ⚠️ **P46 inherits
the crossing cap** — P45 raised it to 65,536 for `9⁵ = 59,049`, but a tenth win-rate rung is
`10⁵ = 100,000` and breaks it again; the decision recurs and must be a stated one.
