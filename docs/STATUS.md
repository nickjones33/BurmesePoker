...
`RULES.md` stays rev 31, the tree is green at **928**, and ⚠️ **not one file in this repository
...
- ⚠️ **(7) One acceptance is outstanding and it is the same sitting for three packets.** A person
  in a browser: **P53**'s round from a phone off the home wifi with the network tab open (the
  credential-replay question, and *is the transport a WebSocket* is now the only thing it decides),
  **P54**'s copy-link button, and **P56**'s advanced opponent group and two-step per-seat form.
  🔥 **The packet stays ◐ for exactly that** — everything a session can check is checked.
- ⚠️ **(8) The basicauth password is in the 2026-08-30 session transcript.** It was never written
