...
- ⚠️ **(5) Outstanding, and it is the acceptance's own last line: the deployed site has not been
  re-tested.** `git push gitea main` is the CI trigger and the role's `pull: true` takes the image
...
- 🔥 **(8) The finding that cost the most, found by pressing the form: the deployed image is not
  ...
  ⚠️ **`git push gitea main` is the CI trigger and it has not been run**, so a packet marked done
...
sitting** — the fix has not been seen on the deployed site, and **`git push gitea main` is the CI
trigger**.
