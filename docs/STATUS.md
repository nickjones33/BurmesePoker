...
🔥 **`P48` shipped 2026-08-27 on Opus 4.8: the verification-and-hardening run — the statistical
review's findings F1–F7 discharged, and `sprinter`'s edge graduated from *measured* to *settled*
at a fresh seed.** `RULES.md` stays rev 31 (no rule changed), the tree is green at **906**, and
**`P49` (`docs/SIMULATIONS.md`, the measurement programme taught) is the next packet** — then `P50`
(the documentation cleanup); `P40` stands beside them, blocked on Nick's vetted Burmese text. ⚠️
**This is not a rung packet**: no agent changed, `BotCatalog` is untouched, and the whole diff is
`BurmesePoker.Sim` + tests + two docs — so `Domain`, `Presentation`, `Console` and `Web` are
byte-identical (the front ends cannot see it). ⚠️ **It cost the whole session in compute**: the
suite is **~5 h wall on this 24-core box** and the fresh-seed replication is another **~4 h** (both
re-run the expensive `outs`-family ladder), so budget a full day for a run like this and never
trust a pasted "~20 min".
